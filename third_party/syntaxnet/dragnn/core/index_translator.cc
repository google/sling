...
Index IndexTranslator::Translate(int batch_index, int feature_value) {
  VLOG(2) << "Translation requested (type: " << method_ << ") for batch "
          << batch_index << " feature " << feature_value;
  Index translated_index;
  translated_index.batch_index = batch_index;
  translated_index.step_index = step_lookup_(batch_index, feature_value);

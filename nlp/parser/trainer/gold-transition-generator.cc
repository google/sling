...
std::vector<string> GoldTransitionSequence::AsStrings(Store *store) const {
...
int GoldTransitionSequence::FirstAction(int token) const {
...
void GoldTransitionGenerator::AttentionIndex::Add(Handle handle) {
...
void GoldTransitionGenerator::AttentionIndex::MakeCenter(Handle handle) {
...
void GoldTransitionGenerator::AttentionIndex::Update(
    const GoldAction &action) {
  switch (action.type) {
...
void GoldTransitionGenerator::InitInfo(const Frame &frame,
...
ParserAction GoldTransitionGenerator::AttentionIndex::Translate(
    const Document &document,
    const GoldAction &gold) {
  ParserAction action;
  action.type = gold.type;
  switch (gold.type) {
    case ParserAction::EVOKE: {
      CHECK_NE(gold.length, 0);
      action.length = gold.length;
      action.label = gold.frame->first_type;
      break;
    }
    case ParserAction::REFER: {
      CHECK_NE(gold.length, 0);
      action.length = gold.length;
      action.target = Index(gold.frame->handle);
      SetMaxIndex(action.target);
      break;
    }
    case ParserAction::EMBED: {
      action.label = gold.frame->first_type;
      action.role = gold.role;
      action.target = Index(gold.other_frame->handle);
      SetMaxIndex(action.target);
      break;
    }
    case ParserAction::ELABORATE: {
      action.label = gold.frame->first_type;
      action.role = gold.role;
      action.source = Index(gold.other_frame->handle);
      SetMaxIndex(action.source);
      break;
    }
    case ParserAction::CONNECT: {
      action.source = Index(gold.frame->handle);
      action.target = Index(gold.other_frame->handle);
      action.role = gold.role;
      SetMaxIndex(action.source);
      SetMaxIndex(action.target);
...
void GoldTransitionGenerator::Generate(
    const Document &document,
    GoldTransitionSequence *sequence,
    GoldTransitionGenerator::Report *report) const {
...
void GoldTransitionGenerator::Generate(
  ...
  std::vector<GoldAction> gold_stack;
  HandleSet evoked;
  for (Span *span : spans_with_frames) {
    ...
    for (int i = previous_start; i < span->begin(); ++i) {
      gold_stack.emplace_back(ParserAction::SHIFT);
    }
    ...
    for (const Slot &slot : span->mention()) {
      if (slot.name == n_evokes_.handle()) {
        ...
        if (evoked.count(slot.value) == 0) {
          gold_stack.emplace_back(ParserAction::EVOKE, f, span->length());
          evoked.insert(slot.value);
        } else {
          gold_stack.emplace_back(ParserAction::REFER, f, span->length());
        }
      }
    }
  }
  ...
  gold_stack.emplace_back(ParserAction::STOP);

  // Reverse 'gold_stack' and treat it as a stack.
  ...
  // of evoked frames.
  std::reverse(gold_stack.begin(), gold_stack.end());

  AttentionIndex attention;
  while (!gold_stack.empty()) {
    GoldAction gold = gold_stack.back();
    gold_stack.pop_back();

    // Translate the gold action using real attention indices and output it.
    sequence->Add(attention.Translate(document, gold));

    // Update the attention index by performing the gold action.
    attention.Update(gold);

    // New frame creation can trigger more actions. Add them to the gold stack.
    if (gold.type == ParserAction::EVOKE ||
        gold.type == ParserAction::EMBED ||
        gold.type == ParserAction::ELABORATE) {
      gold.frame->output = true;

      // Process CONNECT actions.
      for (auto &edge : gold.frame->edges) {
        ...
        if (neighbor_info->output) {
          ...
          gold_stack.emplace_back(ParserAction::CONNECT);
          GoldAction &action = gold_stack.back();
          action.frame = edge.incoming ? neighbor_info : gold.frame,
          action.role = edge.role;
          action.other_frame = edge.incoming ? gold.frame : neighbor_info;
          edge.used = true;
      ...
      for (auto &edge : gold.frame->edges) {
        ...
        if (!neighbor_info->output && edge.incoming) {
          ...
          if (range.begin() == range.end()) {
            gold_stack.emplace_back(ParserAction::EMBED);
            GoldAction &action = gold_stack.back();
            action.frame = neighbor_info;
            action.role = edge.role;
            action.other_frame = gold.frame;
            edge.used = true;
            neighbor_info->edges[edge.inverse].used = true;
          }
        }
      ...
      for (auto &edge : gold.frame->edges) {
        ...
        if (range.begin() == range.end() &&
            ...
            !edge.incoming) {
          gold_stack.emplace_back(ParserAction::ELABORATE, neighbor_info);
          GoldAction &action = gold_stack.back();
          action.role = edge.role;
          action.other_frame = gold.frame;
          edge.used = true;
      ...
      for (auto &edge : gold.frame->edges) {
        ...
        if (!neighbor.IsLocal()) {
          ...
          gold_stack.emplace_back(ParserAction::ASSIGN, gold.frame);
          GoldAction &action = gold_stack.back();
          action.role = edge.role;
          action.value = edge.neighbor;
          edge.used = true;
        }
      }
    }
